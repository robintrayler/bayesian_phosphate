...
# load the white shark data ---------------------------------------------------
data <- read_csv(file = './data/data.csv') %>%
  filter(species == 'carcharias') 
...
results <- model_water_temperature(
  ...
  burn = burn,
  equation = 'kolodny') %>% 
  add_column(group = 'carcharias_ungrouped')


# collapse back into a data frame

write_csv(x = results, file = './data/carcharias_ungrouped_posterior.csv')
